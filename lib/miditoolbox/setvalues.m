function nm = setvalues(nmat,dim,val,ttype);
% Sets the chosen notematrix value for every event
% nm = setvalues(nmat,dim,val,<ttype>);
% Sets the chosen notematrix value for every event
%
% Input arguments:
%	NMAT = notematrix
...
%	VAL =  Value
%	TTYPE = (optional) time representation, 'beat' (default) or 'sec' 
%
% Output:
...
if strcmp(ttype,'beat')+strcmp(ttype,'sec')==0
		disp(['Unknown ttype:' ttype])
